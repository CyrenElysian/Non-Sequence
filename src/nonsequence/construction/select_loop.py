"""Introduce selection and loop structures with an LLM."""
...
DEFAULT_PROMPT: Final = PROJECT_ROOT / "prompts" / "construction" / "prompt_intro.txt"
DEFAULT_INPUT: Final = INTERIM_DIR / "llm_fixed_dev.json"
DEFAULT_OUTPUT: Final = INTERIM_DIR / "processed_data.json"
DEFAULT_LOG: Final = INTERIM_DIR / "change_log.json"
DEFAULT_CHECKPOINT: Final = INTERIM_DIR / "checkpoint.json"
...
def main() -> int:
    """Run the selection-and-loop construction pipeline."""
    args = parse_args()
    ...
    try:
        run_batch_pipeline(
            ...
            user_instruction=(
                "Process every item according to the rules and return the specified result."
            ),
            reasoning=True,
        )
